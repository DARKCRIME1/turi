...
Amdi.applyCMD({pattern: 'up$', fromMe: true, desc: Lang.UPDATER_DESC, dontAddCommandList: true,  deleteCommand: false}, (async (message, match) => {
    await git.fetch();
...
Amdi.applyCMD({pattern: 'uptrex$', fromMe: true, desc: Lang.UPDATE_NOW_DESC, dontAddCommandList: true}, (async (message, match) => {
    await git.fetch();
    var commits = await git.log([Config.BRANCH + '..origin/' + Config.BRANCH]);
    if (commits.total === 0) {
